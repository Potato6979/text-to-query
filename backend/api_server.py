...
from .config import NEO4J_PASSWORD, NEO4J_URI, NEO4J_USER, SCHEMA_INDEX_DIR
...
def _load_pickle_list(path: str | Path) -> list[dict[str, Any]]:
...
def _sql_metadata_records() -> list[dict[str, Any]]:
    return _load_pickle_list(SCHEMA_INDEX_DIR / "metadata.pkl") + paired_sql_metadata_records()


def _cypher_metadata_records() -> list[dict[str, Any]]:
    return _load_pickle_list(SCHEMA_INDEX_DIR / "cypher_metadata.pkl") + paired_cypher_metadata_records()
...
def health():
    ...
    checks: dict[str, Any] = {
        ...
        "sql_index": (SCHEMA_INDEX_DIR / "faiss.index").exists(),
        "cypher_index": (SCHEMA_INDEX_DIR / "cypher_faiss.index").exists(),
    }
    try:
